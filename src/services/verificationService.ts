...
export async function checkOriginality(
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a content verification agent for a digital marketplace called AtriumMind. Your job is to verify that submitted content metadata (title, description, and resource reference) represents a legitimate digital resource that a creator would sell.
